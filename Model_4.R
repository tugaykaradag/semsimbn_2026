...
C =~ 0.8*C1 + 0.8*C2 + 0.8*C3
D =~ 0.8*D1 + 0.8*D2 + 0.8*D3
E =~ 0.8*E1 + 0.8*E2 + 0.8*E3

# Structural model (Model4: D->A, A->E, E->C, E->B, C->B)
A ~ 0.6*D
E ~ 0.6*A
C ~ 0.6*E
B ~ 0.6*E + 0.4*C
...
# (Optional) indicator residual variances (loading=0.8 => residual var=1-0.8^2=0.36)
A1 ~~ 0.36*A1
A2 ~~ 0.36*A2
A3 ~~ 0.36*A3
B1 ~~ 0.36*B1
...
# Structural model (free regressions) - Model4
A ~ D
E ~ A
C ~ E
B ~ E + C
...
aggregate_to_latent_means <- function(df) {
  data.frame(
    A = rowMeans(df[, c("A1","A2","A3")], na.rm = TRUE),
    B = rowMeans(df[, c("B1","B2","B3")], na.rm = TRUE),
    C = rowMeans(df[, c("C1","C2","C3")], na.rm = TRUE),
...
all_edges <- all_edges[!grepl("(A A|B B|C C|D D|E E)", all_edges)]
...
final_table_200
library(writexl)
write_xlsx(final_table_200,"D:/Makaleler/SEM Simulasyon/Revs/Rev13 Array/Added Analyses/model_4_final_table_200.xlsx")
}
#500 icin
{
...
final_table_500
library(writexl)
write_xlsx(final_table_500,"D:/Makaleler/SEM Simulasyon/Revs/Rev13 Array/Added Analyses/model_4_final_table_500.xlsx")
}
#1000 icin
{
...
final_table_1000
library(writexl)
write_xlsx(final_table_1000,"D:/Makaleler/SEM Simulasyon/Revs/Rev13 Array/Added Analyses/model_4_final_table_1000.xlsx")
}
#2500 icin
{
model_pop <- '
...
final_table_2500
library(writexl)
write_xlsx(final_table_2500,"D:/Makaleler/SEM Simulasyon/Revs/Rev13 Array/Added Analyses/model_4_final_table_2500.xlsx")
